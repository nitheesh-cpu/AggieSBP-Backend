...
def get_all_courses() -> List[Tuple[str, str]]:
    ...
    try:
        query = text("""
            SELECT subject_id, course_number 
            FROM courses 
            WHERE subject_id IS NOT NULL 
            AND course_number IS NOT NULL
            AND course_number ~ '^[0-9]+$'
            GROUP BY subject_id, course_number
            ORDER BY subject_id, CAST(course_number AS INTEGER)
        """)

        result = session.execute(query)
        courses = [(row.subject_id, row.course_number) for row in result]
...
async def fetch_course_data(
    session: aiohttp.ClientSession, dept: str, number: str, semaphore: asyncio.Semaphore
...
async def main_async():
    ...
    try:
        ...
        print(
            f"⚡ Speed: {len(all_courses) / duration.total_seconds():.2f} courses/second"
        )
