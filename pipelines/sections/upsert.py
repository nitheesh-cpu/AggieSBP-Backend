...
from pipelines.sections.schemas import (
    SectionSchema, TermSchema,
    SectionDetailsSchema, SectionAttributeDetailedSchema,
    SectionPrereqSchema, SectionRestrictionSchema, SectionBookstoreLinkSchema
)
from pipelines.sections.scraper import get_all_sections, get_all_terms, fetch_section_details_sync
...
def upsert_terms(
    ...
    try:
        ...
        for term in terms:
            ...
            if term.start_date:
                try:
                    start_date = dt.fromisoformat(term.start_date.replace("Z", "+00:00"))
    ...
    try:
        ...
        for section in sections:
            ...
            for meeting in section.meetings:
                meeting_id = f"{section.term_code}_{section.crn}_{meeting.meeting_index}"
                meeting_records.append({
                    ...
                    "days_of_week": meeting.days_of_week if meeting.days_of_week else [],
        ...
        if section_records:
            ...
            for i in range(0, len(section_records), batch_size):
                ...
                try:
                    ...
                    session.rollback()
                    results["errors"].append(f"Error upserting sections batch {i//batch_size}: {e}")
                    continue
        ...
        if instructor_records:
            ...
            for i in range(0, len(instructor_records), batch_size):
                ...
                try:
                    ...
                    session.rollback()
                    results["errors"].append(f"Error upserting instructors batch {i//batch_size}: {e}")
                    continue
        ...
        if meeting_records:
            ...
            for i in range(0, len(meeting_records), batch_size):
                ...
                try:
                    ...
                    session.rollback()
                    results["errors"].append(f"Error upserting meetings batch {i//batch_size}: {e}")
                    continue
...
def upsert_all_sections(
    ...
    all_terms = get_all_terms(current_only=not include_all_terms, semester_filter=semester_filter)
    if all_terms:
        session = get_session()
        try:
    ...
    try:
        ...
        results["terms_upserted"] = term_result.get("terms_upserted", 0) if all_terms else 0
        
        print(f"Done: {results['sections_upserted']} sections, "
...
def upsert_section_details(
    details_list: List[SectionDetailsSchema],
    session=None
) -> Dict[str, int]:
    """
        ...
        if attribute_records:
            ...
            for i in range(0, len(attribute_records), batch_size):
                ...
                try:
                    ...
                    session.rollback()
                    results["errors"].append(f"Error upserting attributes batch {i//batch_size}: {e}")
                    continue
            session.commit()
        ...
        if prereq_records:
            ...
            for i in range(0, len(prereq_records), batch_size):
                ...
                try:
                    ...
                    session.rollback()
                    results["errors"].append(f"Error upserting prereqs batch {i//batch_size}: {e}")
                    continue
            session.commit()
        ...
        if restriction_records:
            ...
            for i in range(0, len(restriction_records), batch_size):
                ...
                try:
                    ...
                    session.rollback()
                    results["errors"].append(f"Error upserting restrictions batch {i//batch_size}: {e}")
                    continue
            session.commit()
        ...
        if bookstore_records:
            ...
            for i in range(0, len(bookstore_records), batch_size):
                ...
                try:
                    ...
                    session.rollback()
                    results["errors"].append(f"Error upserting bookstore links batch {i//batch_size}: {e}")
                    continue
            session.commit()
...
def upsert_all_section_details(
    ...
    try:
        ...
        print(f"Done: {results['attributes_upserted']} attributes, "
...
def delete_old_sections(
    ...
    try:
        ...
        session.execute(
            text("DELETE FROM section_attributes_detailed WHERE term_code NOT IN :terms"),
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Fetch and upsert section data from Howdy API")
    parser.add_argument("--terms", nargs="+", help="Specific term codes to fetch (e.g., 202611 202621)")
    parser.add_argument("--semesters", nargs="+", help="Semester filters (e.g., 'Spring 2026' 'Fall 2025')")
    parser.add_argument("--workers", type=int, default=4, help="Number of parallel workers")
    parser.add_argument("--list-terms", action="store_true", help="List available terms and exit")
    parser.add_argument("--cleanup", action="store_true", help="Delete sections for old terms")
    parser.add_argument("--details", action="store_true", help="Also fetch and upsert section details (slower)")
    parser.add_argument("--details-only", action="store_true", help="Only fetch section details (skip main sections)")
    parser.add_argument("--concurrent", type=int, default=50, help="Max concurrent requests for details")
