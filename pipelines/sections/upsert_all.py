...
from pipelines.sections.scraper import get_all_terms, get_all_sections, fetch_section_details_sync
from pipelines.sections.upsert import (
    upsert_terms, upsert_sections, upsert_section_details
)
    ...
    try:
        ...
        if not details_only:
            ...
            if sections_by_term:
                ...
                results["sections_upserted"] = section_result.get("sections_upserted", 0)
                results["instructors_upserted"] = section_result.get("instructors_upserted", 0)
                results["meetings_upserted"] = section_result.get("meetings_upserted", 0)
        ...
        if include_details or details_only:
            ...
            if sections:
                ...
                results["attributes_upserted"] = details_result.get("attributes_upserted", 0)
                results["prereqs_upserted"] = details_result.get("prereqs_upserted", 0)
                results["restrictions_upserted"] = details_result.get("restrictions_upserted", 0)
                results["bookstore_links_upserted"] = details_result.get("bookstore_links_upserted", 0)
...
"""
    ...
    parser.add_argument("--terms", nargs="+", help="Specific term codes (e.g., 202611 202621)")
    parser.add_argument("--semesters", nargs="+", help="Semester filters (e.g., 'Spring 2026')")
    parser.add_argument("--workers", type=int, default=4, help="Parallel workers for section fetching")
    parser.add_argument("--concurrent", type=int, default=50, help="Max concurrent requests for details")
    parser.add_argument("--details", action="store_true", help="Also fetch section details")
    parser.add_argument("--details-only", action="store_true", help="Only fetch details, skip basic data")
