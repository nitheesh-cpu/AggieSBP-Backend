...
class HierarchicalSummarizer:
    ...
    def _summarize_text(self, text: str, max_length: int = 102, min_length: int = 56) -> str:
        """
    ...
    def _extractive_summary(self, reviews: List[ProcessedReview], max_sentences: int = 3) -> str:
        """
    ...
    def summarize_cluster(
        ...
        if len(cluster_reviews) < 3:
            ...
            academic_phrases = ["in this paper", "we study", "we show", "we propose", "abstract", "introduction to"]
            if any(phrase in summary.lower() for phrase in academic_phrases):
                print(f"Warning: Summary contains academic phrases, using extractive fallback")
                summary = self._extractive_summary(cluster_reviews, max_sentences=4)
        
        # Determine sentiment (simple heuristic)
        text_lower = combined_text.lower()
        positive_words = ["good", "great", "excellent", "amazing", "love", "best", "helpful", "clear"]
        negative_words = ["bad", "terrible", "awful", "worst", "hate", "confusing", "unclear", "difficult"]
