...
def upsert_professor_summary(
    ...
    try:
        ...
        strengths = (
            professor_summary.strengths
            if professor_summary.strengths is not None
            else []
        )
        complaints = (
            professor_summary.complaints
            if professor_summary.complaints is not None
            else []
        )
        ...
        records_to_upsert.append(
            {
                ...
                "tag_frequencies": professor_summary.tag_frequencies,
                "created_at": now,
                "updated_at": now,
            }
        )
        ...
        for course_summary in professor_summary.course_summaries:
            course_id = generate_summary_id(
                professor_summary.professor_id, course_summary.course
            )
            records_to_upsert.append(
                {
                    "id": course_id,
                    ...
                    "strengths": [],  # Empty list, not None
                    "complaints": [],  # Empty list, not None
                    "consistency": None,
                    ...
                    "avg_rating": course_summary.avg_rating,
                    "avg_difficulty": course_summary.avg_difficulty,
                    "common_tags": course_summary.common_tags or [],
                    "tag_frequencies": course_summary.tag_frequencies,
                    "created_at": now,
                    "updated_at": now,
                }
            )
        ...
        try:
            ...
            stmt = stmt.on_conflict_do_update(
                ...
                set_={
                    ...
                    "common_tags": stmt.excluded.common_tags,
                    "tag_frequencies": stmt.excluded.tag_frequencies,
        ...
        if current_course_codes:
            try:
                session.query(ProfessorSummaryNewDB).filter(
                    ProfessorSummaryNewDB.professor_id
                    == professor_summary.professor_id,
        ...
        print(
            f"Error upserting professor summary for {professor_summary.professor_id}: {e}"
        )
        import traceback
...
def get_professor_summary(
    ...
    try:
        # Get overall summary
        overall_record = (
            session.query(ProfessorSummaryNewDB)
            .filter(
        ...
        for professor_id in professor_ids:
            try:
                ...
                if success:
                    results["processed"] += 1
                    print(
                        f"Successfully processed and saved summary for professor {professor_id}"
                    )
                else:
                    results["failed"] += 1
