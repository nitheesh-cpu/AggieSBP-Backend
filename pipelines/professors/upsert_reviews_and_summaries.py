...
def upsert_reviews_and_summaries(
    ...
    try:
        ...
        remaining_professors = [pid for pid in professor_ids if pid not in processed_professors]
        ...
        for batch_num in range(total_batches):
            ...
            if skip_if_no_new_reviews:
                ...
                professors_to_summarize = [
                    prof_id for prof_id in batch_professors
                    if prof_id in new_reviews_dict or prof_id not in processed_professors
                ]
            else:
            ...
            for i, professor_id in enumerate(professors_to_summarize, 1):
                try:
                    print(f"    [{i}/{len(professors_to_summarize)}] {professor_id[:20]}...", end=" ", flush=True)
                    
                    # Get all reviews for this professor
                    all_reviews = session.query(ReviewDB).filter_by(
