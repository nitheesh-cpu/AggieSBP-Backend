...
from database.base import ProfessorDB, ReviewDB, get_session, upsert_reviews
    ...
    async with semaphore:
        ...
        with ThreadPoolExecutor() as executor:
            reviews = await loop.run_in_executor(
                executor, collector.get_all_reviews, professor.id
            )

        if reviews:
            print(
                f"✅ {professor.first_name} {professor.last_name}: {len(reviews)} reviews"
            )
            return reviews
        else:
...
async def collect_all_reviews_async(
    collector, professors, session, max_concurrent=15, batch_size=10000
):
    ...
    print(
        f"🚀 Starting concurrent review collection with max {max_concurrent} concurrent requests..."
    )
    print(f"📦 Will batch upsert every {batch_size:,} reviews")

    # Create tasks for all professors
    tasks = [
        collect_reviews_for_professor(collector, prof, semaphore) for prof in professors
    ]
    ...
    for i, result in enumerate(results):
        if isinstance(result, Exception):
            print(
                f"❌ Error for {professors[i].first_name} {professors[i].last_name}: {result}"
            )
            failed_profs += 1
        else:
            reviews = result
            if reviews:
                ...
                if len(all_reviews) >= batch_size:
                    print(
                        f"📦 Batch {batch_count}: Upserting {len(all_reviews):,} reviews..."
                    )
                    try:
                        upsert_reviews(session, all_reviews)
                        print(
                            f"✅ Batch {batch_count}: Successfully upserted {len(all_reviews):,} reviews"
                        )
    ...
    if all_reviews:
        print(
            f"📦 Final batch {batch_count}: Upserting {len(all_reviews):,} reviews..."
        )
        try:
            upsert_reviews(session, all_reviews)
            print(
                f"✅ Final batch {batch_count}: Successfully upserted {len(all_reviews):,} reviews"
            )
        except Exception as e:
...
async def get_missing_professors(session):
    ...
    professors_without_reviews = [
        prof for prof in professors if prof.id not in professors_with_reviews
    ]
